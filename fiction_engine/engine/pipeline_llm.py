...
def parse_first_item(text: str, header: str) -> str:
    ...
    # Запасной путь: критик иногда перестраивает ответ и перечисляет
    # претензии не под своим заголовком, а в разделе чеклиста:
    #
    #     ## ЧЕКЛИСТ ЗДОРОВЬЯ СЦЕНЫ
    #     **Scene Health — критические нарушения:**
    #     - ✗ Глава не меняет состояние персонажа/мира
    #
    # Замер 14.09 на настоящих ответах: так выходит примерно в 1 случае
    # из 6, и в этих случаях автор не видел претензии вовсе. Это остаток
    # того же дефекта, что чинился утром (коммит 2555cd1): тогда разбор
    # не совпадал никогда, теперь — иногда.
    m = re.search(r"(?:наруш|проблем|замечан)[^\n]*:?\s*\n+\s*(?:[-*•✗✘×]|\d+[.)])\s*(.+)",
                  plain, re.IGNORECASE)
