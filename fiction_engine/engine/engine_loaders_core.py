...
def _read_md_useful(path: Path, max_lines: int) -> list[str]:
    """
    Читает .md файл и возвращает значимые строки.

    Шапка (метаданные до разделителя ---) пропускается, а вот содержимое
    code-блоков СОХРАНЯЕТСЯ, хотя ограждающие ``` убираются. Это намеренно:
    в базе знаний внутри блоков лежат примеры прозы «✅ ХОРОШО / ❌ ПЛОХО» —
    579 таких блоков против 457 технических. Выбросить их значило бы
    выбросить самое ценное, ради чего база и собрана.

    Прежний докстринг утверждал обратное («пропуская … code-блоки») и
    расходился с кодом; сохранено поведение, исправлено описание.
    """
