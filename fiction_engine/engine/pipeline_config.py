...
# Пороги измерены на claude-haiku-4-5 (14.09) тем же способом: 15 настоящих
# глав против 15 нарочно испорченных, порог обязан принимать целое и
# отвергать испорченное.
#
#      итог  критерий   проходят настоящие   протекают испорченные
#        25       —          11 из 15             10 из 15   ← бессмыслен
#        30       4           7 из 15              2 из 15   ← выбрано
#        32       —           2 из 15              2 из 15   ← режет живое
#        40       7           0 из 15              0 из 15   ← прежний
#
# Порог 30 выше медианы настоящих текстов (29): «ПРИНЯТЬ» значит «лучше
# обычного», а не «как обычно».
#
# ИСТОРИЯ СМЕН СУДЬИ, чтобы числа не разъехались снова. 15.09 судья был
# переведён на kimi-k2.5 (кончились кредиты Anthropic) и пороги
# перекалиброваны на него — 30 и 5. 22.09 кредиты пополнены, судьёй снова
# Haiku, и пороги возвращены к измеренным на нём: 30 и 4.
#
# Перекалибровка на claude-sonnet-5 НЕ ПРОВЕДЕНА: два захода сорвались о
# дефекты (блок размышления съедал бюджет ответа), а после их починки
# Sonnet поставлен писателем, а не судьёй. Ставить его судьёй без
# калибровки нельзя — тридцать у одной модели и тридцать у другой это
# разные тридцать.
ACCEPT_TOTAL         = 30    # из 50
ACCEPT_MIN_CRITERION = 4     # из 10, по каждому из пяти

# Судья, на котором калиброваны числа выше. Менять его без перекалибровки
# нельзя: тридцать у одной модели и тридцать у другой — разные тридцать.
#
# Выбор 15.09 между двумя доступными кандидатами:
#
#   glm-5.3     различает вдвое лучше (разрыв средних 10.3 против 4.3),
#               но проваливает каждый пятый вызов пустым ответом и ставит
#               своё семейство на три верхних места из четырёх
#   kimi-k2.5   0 отказов на 30 вызовов, самопредпочтения не видно,
#               но сжимает оценки в полосу 24-32
#
# Взята надёжность: судья работает на каждой главе, и пропадающая оценка
# в этом движке превращается в ноль, неотличимый от плохого текста.
#
