...
# ПЕРЕКАЛИБРОВАНО 15.09 на новом судье. Прежние 30/4 получены на
# claude-haiku-4-5; прямой ключ Anthropic исчерпан, и судья заменён на
# moonshotai/kimi-k2.5. Числа совпали не случайно, но и не автоматически —
# калибровка проведена заново тем же способом (15 настоящих глав против 15
# нарочно испорченных):
#
...
#
# Порог 30 выше медианы настоящих текстов (27), поэтому «ПРИНЯТЬ» значит
# «лучше обычного». Планка критерия поднята с 4 до 5: на этом судье она
# работает — при пороге 28 одна она отсекает две утечки из трёх.
ACCEPT_TOTAL         = 30    # из 50
ACCEPT_MIN_CRITERION = 5     # из 10, по каждому из пяти
