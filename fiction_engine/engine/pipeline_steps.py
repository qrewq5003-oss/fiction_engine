...
# Порог берётся из pipeline_config, а не вписывается числом: раньше он стоял
# в двух местах разными значениями (40 в правиле вердикта, 38 в профилях
# пайплайна). Такие копии в этом проекте уже расходились дважды.
from .pipeline_config import (ACCEPT_TOTAL as _accept_total,
                              ACCEPT_MIN_CRITERION as _accept_min)

SYS_JUDGE = f"""Ты — главный редактор серии. Сравниваешь два варианта текста и выносишь финальный вердикт.
...
ПРАВИЛО ВЕРДИКТА: ПРИНЯТЬ только если ИТОГ ≥ {_accept_total} И каждый из 5 критериев ≥ {_accept_min} И ни одного нарушения Scene Health чеклиста. Если хоть одно условие не выполнено — НА ДОРАБОТКУ. Не делай исключений.
